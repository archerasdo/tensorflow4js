...
function topK(values: Float32Array, k: number):
  ...
  const kRankValue = valuesAndIndices[k].value
  // 特征相同 排序需要等距抽样
  if (kRankValue == valuesAndIndices[0].value) {
    const equalDistanceList: Array<{value: number, index: number}> = 
      valuesAndIndices.filter(el => el.value == kRankValue)
    const equalDistanceListLength: number = equalDistanceList.length
    const frequency: number = Math.floor(equalDistanceListLength/k)

    return equalDistanceList
      .filter((el, index) => index % frequency === 0) 
  } else {
    return valuesAndIndices.filter((el, index) => index < k)
  }
}
...
export class KNNClassifier {
  ...
  async predictClass(input: Tensor, k = 3):
    ...
    const topKItem = topK(await knn.data() as Float32Array, kVal);

    knn.dispose();

    return this.calculateTopClass(topKItem, kVal);
  }
   ...
   */
  private calculateTopClass(topKItem: Array<{value: number, index: number}>, kVal: number) {
    let exampleClass = -1;
    ...
    const totalWeight = topKItem.reduce((ret, el) => ret + el.value, 0)
    ...
    for (let i = 0; i < topKItem.length; i++) {
      const { index: topkIndices, value: weight } = topKItem[i]
    ...
    let topConfidence = 0;
    for (const i in this.classDatasetMatrices) {
      const probability = totalWeight > 0 ? topKCountsForClasses[i] / totalWeight : 0 ;
